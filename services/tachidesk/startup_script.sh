...
echo "Suwayomi data location inside the container: $work_dir/.local/share/Tachidesk"
...
envsubst < $work_dir/server.conf.template > $work_dir/.local/share/Tachidesk/server.conf

exec java -jar "$work_dir/tachidesk_latest.jar";